...
public class SideScrollPlayer : MonoBehaviour
{
    ...
    [SerializeField] private float jumpStaminaCost = 10f; // Thêm quản lý stamina cho nhảy
    ...
    {
        if (activeInput.attack1Pressed) { bufferedAttackType = 1; attackBufferCounter = attackBufferTime; }
        else if (activeInput.attack2Pressed) { bufferedAttackType = 2; attackBufferCounter = attackBufferTime; }
        else if (activeInput.attack3Pressed) { bufferedAttackType = 3; attackBufferCounter = attackBufferTime; }
    ...
    private void TryExecuteAttack(float staminaCost, System.Action animationAction)
    {
        ...
        {
            Debug.Log("<color=orange>[ACTION FAILED]:</color> Không đủ Stamina để chém!");
        }
    }
    ...
    public void TriggerPlayerAttackHitbox()
    {
        ...
        foreach (Collider2D enemy in hitEnemies)
        {
            if (enemy.TryGetComponent<IDamageable>(out var damageableTarget))
            {
                ...
                Debug.Log($"<color=green>[PLAYER ATTACK HIT]:</color> Chém trúng {enemy.name} gây {currentAttackDamage} dmg!");
            }
        }
    }
    #endregion
    ...
    private void UpdatePlayerState()
    {
        ...
        {
            ChangeState(activeInput.isRunning && status.HasEnoughStamina(runStaminaCost * Time.deltaTime) ? PlayerState.Running : PlayerState.Walking);
        }
        else
        {
            ChangeState(PlayerState.Idle);
    ...
    private void HandleJump()
    {
        ...
        if (jumpBufferCounter > 0f && coyoteTimeCounter > 0f)
        {
            ...
            {
                Debug.Log("<color=orange>[ACTION FAILED]:</color> Không đủ Stamina để nhảy!");
            }
        }
    }
    ...
    private void UpdateAnimatorSpeed()
    {
        ...
        float animSpeed = Mathf.Abs(activeInput.moveX) > 0.01f ? (activeInput.isRunning && status.HasEnoughStamina(runStaminaCost * Time.deltaTime) ? 2f : 1f) : 0f;
